...
data "azurerm_managed_disk" "os_disk" {
  name                = local.os_type == "linux" ? try(azurerm_linux_virtual_machine.vm["linux"].os_disk[0].name, null) : local.os_type == "windows" ? try(azurerm_windows_virtual_machine.vm["windows"].os_disk[0].name, null) : try(azurerm_virtual_machine.vm["legacy"].storage_os_disk[0].name, null)
